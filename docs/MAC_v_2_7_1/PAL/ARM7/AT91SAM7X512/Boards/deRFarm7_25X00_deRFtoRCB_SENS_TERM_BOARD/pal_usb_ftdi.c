 ...
 *
 * $Id: pal_usb_ftdi.c,v 1.1.4.1 2011/07/08 16:55:31 dam Exp $
 *
 * @author
...
#if (BOARD_TYPE == deRFarm7_25X00_deRFtoRCB_SENS_TERM_BOARD)
...
/* set all portpins to input */
 ...
 #define USB_DATA_SETINP()   do{                    \
    ...
    PIN_SET_AS_PIO_INPUT(USB_ADDR_PIN_6, PIO_A);    \
    PIN_SET_AS_PIO_INPUT(USB_ADDR_PIN_7, PIO_A);    \
 }while(0)
#else
 #define USB_DATA_SETINP()   do{                    \
    PIN_SET_AS_PIO_INPUT(USB_ADDR_PIN_0, PIO_B);    \
...
/* set all portpins to output */
 ...
 #define USB_DATA_SETOUTP()   do{                   \
    ...
    PIN_SET_AS_PIO_OUTPUT(USB_ADDR_PIN_6, PIO_A);   \
    PIN_SET_AS_PIO_OUTPUT(USB_ADDR_PIN_7, PIO_A);   \
 }while(0)
#else
 #define USB_DATA_SETOUTP()   do{                   \
    PIN_SET_AS_PIO_OUTPUT(USB_ADDR_PIN_0, PIO_B);   \
...
#if (defined BOARD_REV00)
    if(data & (1<<6))    { PIN_SET(USB_ADDR_PIN_6, PIO_A);     }
    else                 { PIN_CLEAR(USB_ADDR_PIN_6, PIO_A);   }

    if(data & (1<<7))    { PIN_SET(USB_ADDR_PIN_7, PIO_A);     }
    else                 { PIN_CLEAR(USB_ADDR_PIN_7, PIO_A);   }
#else
...
static uint8_t read_byte(void)
{
    ...
    ret |= (status << 5);
#if (defined BOARD_REV00)
    PIN_GET_STATUS(status, USB_ADDR_PIN_6, PIO_A);
    ret |= (status << 6);
    PIN_GET_STATUS(status, USB_ADDR_PIN_7, PIO_A);
    ret |= (status << 7);
#else
...
#endif /* (BOARD_TYPE = deRFarm7_25X00_deRFtoRCB_SENS_TERM_BOARD) */
