 ...
 *
 * $Id: sensors.c,v 1.1.4.1 2011/07/08 16:59:08 dam Exp $
 *
...
twi_status_t BMA150_ConfigureAnymotionIrq(uint8_t enabled)
{
    ...
    if(enabled) {
        ...
        OCR1AL = 1;                  // set compare match value
        TCCR1B = (0x07<<CS10);       // externally clocked, riging edge
        TCCR1B |= _BV(WGM12);        // auto reset counter value upon compare match
        TIMSK1 |= _BV(OCIE1A);       // cmp. match IRQ enable
        sei();                       // globally enable interrupts, if not already done
