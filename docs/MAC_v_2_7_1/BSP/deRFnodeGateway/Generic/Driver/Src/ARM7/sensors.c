 ...
 *
 * $Id: sensors.c,v 1.1.4.1 2011/07/08 16:59:09 dam Exp $
 *
...
#include "bsp_config.h"             // physical interconnection
...
static unsigned char ucBuf[3];                  // buffer used for twi transmissions

// Pin on which BMA150 may trigger interrupts
static const Pin pin_BMA150_IRQ = PIN_BMA150IRQ;

// remember if an PIO IRQ callback has been already registered
static bool bma150_pio_isr_registered = false;
...
twi_status_t TWI_MasterInit(void)
{
    ...
    if(!PIO_Configure(pins, PIO_LISTSIZE(pins))) goto fail;

    // Enable TWI peripheral
    PMC_EnablePeripheral(AT91C_ID_TWI);

    // Initialize the TWI Peripheral
    TWI_ConfigureMaster(AT91C_BASE_TWI, TWI_CLK, BOARD_MCK);
...
twi_status_t DS620_GetTemperature(temperature_t* temperature, bool blocking_wait)
{
    ...
    if(blocking_wait && (CONTINUOUS != ds620_conv_mode)) {
        ...
        } while(cnt++ < ((uint32_t)BOARD_MCK)/5);
    ...
    // assemble transfer buffer and write it
    ucBuf[0] = DS620_ADDR_TEMP_MSB;
    if(TWID_Write(&twid, DS620_ADDRESS, 0, 0, ucBuf, 1, NULL)) goto fail;

    // read 3 bytes and split buffer
    if(TWID_Read(&twid, DS620_ADDRESS, 0, 0, ucBuf, 3, NULL)) goto fail;
    temp_msb = ucBuf[0];
    temp_lsb = ucBuf[1];
    cfg_msb  = ucBuf[2];
...
static twi_status_t DS620_ReadConfigRegister(uint8_t* msb, uint8_t* lsb)
{
    ...
    ucBuf[0] = DS620_ADDR_CONFIG_MSB;
    if(TWID_Write(&twid, DS620_ADDRESS, 0, 0, ucBuf, 1, NULL)) goto fail;

    // read from DS620, split buffer
    if(TWID_Read(&twid, DS620_ADDRESS, 0, 0, ucBuf, 2, NULL)) goto fail;
    *msb = ucBuf[0];
    *lsb = ucBuf[1];
...
static twi_status_t DS620_WriteConfigRegister(uint8_t msb, uint8_t lsb)
{
    ...
    ucBuf[1] = msb;
    ucBuf[2] = lsb;
    if(TWID_Write(&twid, DS620_ADDRESS, 0, 0, ucBuf, 3, NULL)) goto fail;
...
static twi_status_t DS620_SendCommand(uint8_t cmd)
{
    if(TWID_Write(&twid, DS620_ADDRESS, 0, 0, &cmd, 1, NULL)) goto fail;
...
twi_status_t TMP102_GetTemperature(temperature_t* temperature, bool blocking_wait)
{
    ...
    if(blocking_wait && (CONTINUOUS != tmp102_conv_mode)) {
        ...
        } while(cnt++ < BOARD_MCK/32);

        // loop counter exceeded?
        if(!(cfg_msb & TMP102_CFG1_ONESHOT_READY)) goto fail;
    }

    // assemble transfer buffer and write it, then read 3 bytes
    ucBuf[0] = TMP102_ADDR_TEMP;
    if(TWID_Write(&twid, TMP102_ADDRESS, 0, 0, ucBuf, 1, NULL)) goto fail;
    if(TWID_Read(&twid, TMP102_ADDRESS, 0, 0, ucBuf, 3, NULL)) goto fail;
...
{
    // set start address which to read from
    ucBuf[0] = TMP102_ADDR_CFG;
    if(TWID_Write(&twid, TMP102_ADDRESS, 0, 0, ucBuf, 1, NULL)) goto fail;

    // read from device, split buffer
    if(TWID_Read(&twid, TMP102_ADDRESS, 0, 0, ucBuf, 2, NULL)) goto fail;
    *msb = ucBuf[0];
    *lsb = ucBuf[1];
...
static twi_status_t TMP102_WriteConfigRegister(uint8_t msb, uint8_t lsb)
{
    ...
    ucBuf[1] = msb;
    ucBuf[2] = lsb;
    if(TWID_Write(&twid, TMP102_ADDRESS, 0, 0, ucBuf, 3, NULL)) goto fail;
...
twi_status_t ISL29020_GetLuminosity(luminosity_t* luminosity)
{
    ...
    // assemble transfer buffer and write it
    ucBuf[0] = ISL29020_ADDR_DATA_LSB;
    if(TWID_Write(&twid, ISL29020_ADDRESS, 0, 0, ucBuf, 1, NULL)) goto fail;

    // read 2 bytes, LSB (!) is transmitted first
    if(TWID_Read(&twid, ISL29020_ADDRESS, 0, 0, ucBuf, 2, NULL)) goto fail;
    data_lsb = ucBuf[0];
    data_msb = ucBuf[1];
    ...
    *luminosity = ((uint16_t) (data_lsb | data_msb << 8))*((float)RANGE/(float)COUNT);
...
{
    // set start address which to read from
    ucBuf[0] = ISL29020_ADDR_COMMAND;
    if(TWID_Write(&twid, ISL29020_ADDRESS, 0, 0, ucBuf, 1, NULL)) goto fail;

    // read from device
    if(TWID_Read(&twid, ISL29020_ADDRESS, 0, 0, cmd, 1, NULL)) goto fail;
...
static twi_status_t ISL29020_WriteCommandRegister(uint8_t cmd)
{
    ...
    ucBuf[0] = ISL29020_ADDR_COMMAND;
    ucBuf[1] = cmd;
    if(TWID_Write(&twid, ISL29020_ADDRESS, 0, 0, ucBuf, 2, NULL)) goto fail;
...
/**
 * @brief ISR for BMA150-triggered interrupts. Invokes user defined callback
 * if there is any. Remember that depending on the set sensitivity (motion
 * duration/-threshold) these interrupts may occur with high frequency!
 */
void ISR_BMA150(void)
{
    // BMA150 triggers with H level
    if(PIO_Get(&pin_BMA150_IRQ)) {
...
twi_status_t BMA150_ConfigureAnymotionIrq(uint8_t enabled)
{
    ...
    if(enabled) {
        ...
        PIO_Configure(&pin_BMA150_IRQ, 1);
        if(!bma150_pio_isr_registered) {
            PIO_ConfigureIt(&pin_BMA150_IRQ, (void (*)(const Pin *)) ISR_BMA150);
            bma150_pio_isr_registered = true;
        }
        PIO_EnableIt(&pin_BMA150_IRQ);

    } else {
        PIO_DisableIt(&pin_BMA150_IRQ);
    }
...
{
    // set start address which to read from
    if(TWID_Write(&twid, BMA150_ADDRESS, 0, 0, &addr, 1, NULL)) goto fail;

    // read from device
    if(TWID_Read(&twid, BMA150_ADDRESS, 0, 0, val, 1, NULL)) goto fail;
...
static twi_status_t BMA150_WriteRegister(uint8_t addr, uint8_t val)
{
    ...
    ucBuf[0] = addr;
    ucBuf[1] = val;
    if(TWID_Write(&twid, BMA150_ADDRESS, 0, 0, ucBuf, 2, NULL)) goto fail;
