 ...
 * @{
 * @addtogroup macExamples_WUART2 Wireless_UART_2
 * @{
 *
 * @brief  Demo application showing wireless serial-serial communication
 *
 * Improved Wireless UART, based on existing TINY_TAL Example.
 ...
 *
 * $Id: main.c,v 1.1.6.1 2011/07/14 18:37:08 dam Exp $
 *
...
#define UART_ECHO                   (false) // echo back characters after successful transmission ?

#define FRAME_RETRIES               (7)     // see IEEE 802.15.4 Table 86: 0...7
/*
...
void tal_tx_frame_done_cb(retval_t status)
{
    ...
    else if (status == MAC_CHANNEL_ACCESS_FAILURE)
    {
         ...
         *
         * In case of channel access failure the frame is retried (once).
         */

        /* Transmission was not successful, initiate retry. */
        tx_state = TX_RETRY;
    }
    else if (status == TAL_FRAME_PENDING)
    {
        tx_state = TX_ONGOING;
    }
    ...
    {
        /*
         * Transmission was corrupted. Assume that not only the ACK got lost
         * but the recipient didn't get our frame at all - perform a certain
         * amount of retransmission(s), but do not block forever.
         */
        if(retry_count<0xFF)
        {
            // perform retries by the application layer [app_task()]
            tx_state = TX_RETRY;
            retry_count++;
        }
        else
        {
            // transmission finally failed. has to be handled by customer application.
            tx_state = TX_IDLE;
         ...
         * The transmission is considered as being completed for this frame.
         */
    {
        tx_state = TX_IDLE;
...
static void configure_frame_sending(void)
{
    ...
    /* Set amount of frame retries */
    temp_value_8 = FRAME_RETRIES;
