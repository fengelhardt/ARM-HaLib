 ...
 * @{
 * @addtogroup macExamples_deRFsimple_USB deRFsimple_USB
 * @{
 *
 * @brief  Demo application showing USB access using the PAL layer
 * 
...
/** Number of necessary consecutive "button pressed" detections, before a button event is signalled. */
 ...
 */
static void app_read_temp(temperature_t* temperature)
{
#ifdef INTERNAL_TEMP
   /* start conversion */
   ADCSRA |= (1<<ADSC);
 ...
 *
 * This function sends the actual temperature to USB interface
 * and toggles LED0 if a keyboard key is pressed.
 */
...
int main(void)
{
   ...
   while(1)
   {
      ...
      /* calls usb_handler() which checks for USB data */
      pal_task();
   }
}
