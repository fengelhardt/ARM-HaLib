 ...
 *
 * $Id: main.c,v 1.4.2.1 2011/07/14 18:42:33 dam Exp $
 *
...
int main(void)
{
     ...
     * if using an arm-eabi-none-based toolchain, it is required to compile syscalls.c
     */
...
#else
        // the Cortex is not capable of handling floats/double in va-lists
        // so calculate an alternate result by ourselves
        printf("Test duration = %u.%06u s\r\n", (unsigned int) duration/1000000, (unsigned int) duration%1000000);
#endif
...
#else        
        //printf("Net data rate = %" PRIu32 ".%03 " PRIu32" kbit/s\r\n", (unsigned int)data_rate/1000, (unsigned int)data_rate%1000);
        printf("Net data rate = %u.%03u kbit/s\r\n", (unsigned int) data_rate, (unsigned int) (data_rate*1000));
#endif
